...
import { DecreaseOrder } from "../positionOrderTable";
...
import { EditDecreaseBtn } from "../transactions/editDecreaseBtn";
import { CancelDecreaseBtn } from "../transactions/cancelDecreaseBtn";
...
export default function SlOrders({ address, chainConstants, orders, allPrices, setOrderUpdate }: ISlOrders) {
  ...
  return (
    <>
              ...
              return (
                <Tr key={index}>
                  <Td px={{ base: 2, md: 4 }}>
                    ...
                    <Text color={order.isLong ? "green.400" : "red.500"}>{order.isLong ? "Long" : "Short"}</Text>
                  </Td>
                  <Td px={{ base: 2, md: 4 }}>
                    <Text>${formatValue(order.collateral)}</Text>
                  </Td>
      ...
      <Modal isOpen={isEditOpen} onClose={onEditClose}>
        ...
        <ModalContent>
          <ModalHeader>
            Edit {assetLabel} {isLong ? "Long" : "Short"} SL
          </ModalHeader>
          <ModalCloseButton />
          <ModalBody>
      ...
      <Modal isOpen={isCancelOpen} onClose={onCancelClose}>
        ...
        <ModalContent>
          <ModalHeader>
            Cancel {assetLabel} {isLong ? "Long" : "Short"} SL
          </ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <Flex direction={"column"} gap={2} mt={4} backgroundColor={"gray.700"} p={1} rounded={"md"}>
