...
export default function Pools() {
  ...
  useEffect(() => {
    if (chain?.id && address && chainConstants) {
      ...
      const lpUserInfoData: ContractInput[] = tokensList?.map((token, index) => ({
        address: token.mmAddress,
        abi: MasterMantisABI,
        functionName: "userInfo",
        args: [BigInt(token.lpId), address],
      }));
      const lpPendingMntsData: ContractInput[] = tokensList?.map((token, index) => ({
        address: token.mmAddress,
        abi: MasterMantisABI,
      ...
      const plpUserInfoData: ContractInput[] = tokensList?.map((token, index) => ({
        address: token.mmAddress,
        abi: MasterMantisABI,
        functionName: "userInfo",
        args: [BigInt(token.plpId), address],
      }));
      const plpPendingMntsData: ContractInput[] = tokensList?.map((token, index) => ({
        address: token.mmAddress,
        abi: MasterMantisABI,
        functionName: "pendingMnts",
        args: [BigInt(token.plpId), address],
      ...
      const getContracts = async () => {
        ...
        const lpUserdata = await readContracts({
          contracts: [...lpBalanceData, ...lpUserInfoData, ...lpPendingMntsData],
        });
        const plpUserData = await readContracts({
          contracts: [...plpBalanceData, ...plpUserInfoData, ...plpPendingMntsData],
        });

        const volumeData = await axios.get(
          "https://api.mantissa.finance/api/pool/stats/volume/daily/" + chain?.id + "/"
        );

        const updatedTokenList = tokensList?.map((token: Token, index: number, arr: Token[]) => {
          if (
            ...
            (plpData[index].status === "success" && plpData[index + arr.length].status === "success") ||
            (plpUserData[index].status === "success" &&
              plpUserData[index + arr.length].status === "success" &&
              plpUserData[index + arr.length + arr.length].status === "success")
          ) {
            const assetLPResult =
              lpdata[index].status === "success" ? (lpdata[index]?.result as string | BigNumber) : 0;
            ...
            const totalSupplyLPResult =
              lpdata[index + arr.length + arr.length].status === "success"
                ? (lpdata[index + arr.length + arr.length]?.result as string | BigNumber)
                : 0;
            const assetpLPResult =
              plpData[index].status === "success" ? (plpData[index]?.result as string | BigNumber) : 0;
            ...
            const balanceLPResult =
              lpUserdata[index].status === "success" ? (lpUserdata[index]?.result as string | BigNumber) : 0;
            const userInfoLPResult =
              lpUserdata[index + arr.length].status === "success"
                ? ((lpUserdata[index + arr.length]?.result as Array<any>)[0] as string | BigNumber)
                : 0;
            const pendingMntsLPResult =
              lpUserdata[index + arr.length + arr.length].status === "success"
                ? (lpUserdata[index + arr.length + arr.length]?.result as string | BigNumber)
                : 0;
            const balancePLPResult =
              plpUserData[index].status === "success" ? (plpUserData[index]?.result as string | BigNumber) : 0;
            const userInfoPLPResult =
              plpUserData[index + arr.length].status === "success"
                ? ((plpUserData[index + arr.length]?.result as Array<any>)[0] as string | BigNumber)
                : 0;
            const pendingMntsPLPResult =
              plpUserData[index + arr.length + arr.length].status === "success"
                ? (plpUserData[index + arr.length + arr.length]?.result as string | BigNumber)
                : 0;

            const lpAsset =
              typeof assetLPResult === "string" ? BigInt(assetLPResult) : BigInt(assetLPResult.toString());
            const lpLiability =
              typeof libilityLPResult === "string" ? BigInt(libilityLPResult) : BigInt(libilityLPResult.toString());
            const lpTotalSupply =
              typeof totalSupplyLPResult === "string"
                ? BigInt(totalSupplyLPResult)
                : BigInt(totalSupplyLPResult.toString());
            const plpAsset =
              typeof assetpLPResult === "string" ? BigInt(assetpLPResult) : BigInt(assetpLPResult.toString());
            ...
            const lpBalance =
              typeof balanceLPResult === "string" ? BigInt(balanceLPResult) : BigInt(balanceLPResult.toString());
            const lpUserInfo =
              typeof userInfoLPResult === "string" ? BigInt(userInfoLPResult) : BigInt(userInfoLPResult.toString());
            const lpPendingMnts =
              typeof pendingMntsLPResult === "string"
                ? BigInt(pendingMntsLPResult)
                : BigInt(pendingMntsLPResult.toString());
            const plpBalance =
              typeof balancePLPResult === "string" ? BigInt(balancePLPResult) : BigInt(balancePLPResult.toString());
            const plpUserInfo =
              typeof userInfoPLPResult === "string" ? BigInt(userInfoPLPResult) : BigInt(userInfoPLPResult.toString());
            ...
            let tokenVolume = "-";
            try {
              tokenVolume = volumeData.data["token_volume"][token.name.toLowerCase()].toString();
            } catch {}

            return {
              ...token,
              lpAsset: formatUnits(lpAsset, getTokenDecimals(token.name.toLowerCase())),
              lpLiability: formatUnits(lpLiability, getTokenDecimals(token.name.toLowerCase())),
              lpTotalSupply: formatUnits(lpTotalSupply, getTokenDecimals(token.name.toLowerCase())),
              plpAsset: formatUnits(plpAsset, getTokenDecimals(token.name.toLowerCase())),
              plpTotalSupply: formatUnits(plpTotalSupply, getTokenDecimals(token.name.toLowerCase())),
              lpBalance: formatUnits(lpBalance, getTokenDecimals(token.name.toLowerCase())),
              lpUserInfo: formatUnits(lpUserInfo, getTokenDecimals(token.name.toLowerCase())),
              lpPendingMnts: formatUnits(lpPendingMnts, getTokenDecimals(token.name.toLowerCase())),
              plpBalance: formatUnits(plpBalance, getTokenDecimals(token.name.toLowerCase())),
              plpUserInfo: formatUnits(plpUserInfo, getTokenDecimals(token.name.toLowerCase())),
              plpPendingMnts: formatUnits(plpPendingMnts, getTokenDecimals(token.name.toLowerCase())),
              volume: tokenVolume,
            };
          }
          return token;
  ...
  const getLpLiability = (tokenName: string) => {
    return tokenList?.find((token: Token) => tokenName === token.name)?.lpLiability;
  };

  const getLpSupply = (tokenName: string) => {
    return tokenList?.find((token: Token) => tokenName === token.name)?.lpTotalSupply;
  };

  const getPlpAsset = (tokenName: string) => {
    return tokenList?.find((token: Token) => tokenName === token.name)?.plpAsset;
  };

  const getPlpSupply = (tokenName: string) => {
    return tokenList?.find((token: Token) => tokenName === token.name)?.plpTotalSupply;
  };

  const getLpBalance = (tokenName: string) => {
    return tokenList?.find((token: Token) => tokenName === token.name)?.lpBalance;
  };

  const getPlpBalance = (tokenName: string) => {
    return tokenList?.find((token: Token) => tokenName === token.name)?.plpBalance;
  };

  const getLpStake = (tokenName: string) => {
    return tokenList?.find((token: Token) => tokenName === token.name)?.lpUserInfo;
  };

  const getPlpStake = (tokenName: string) => {
    return tokenList?.find((token: Token) => tokenName === token.name)?.plpUserInfo;
  };

  const getMntsEarned = (tokenName: string) => {
    const tokenInfo = tokenList?.find((token: Token) => tokenName === token.name);
  ...
  const lightButtonBg = useColorModeValue("blackAlpha.300", "rgba(255, 255, 255, 0.05)");
  const buttonBgColor = useColorModeValue("blackAlpha.800", "#0080FF");

  return (
    <Box
      w={{ base: "lg", md: "68.688rem", sm: "80%" }}
      mx="auto"
      display="flex"
      flexDir={"column"}
      gap={"1.875rem"}
    ...
    >
      ...
      >
        <VStack borderRadius={"1rem"} p="1rem" gap="1.875rem" bg={boxBg}>
          <HStack w="100%" gap="0.375rem">
            <Box w={"3px"} h="0.875rem" bgColor={"#00A3FF"} borderRadius={"3px"}></Box>
            <HStack gap="0.2rem">
          ...
          <VStack gap="1rem">
            <HStack w="100%" alignItems="baseline">
              <Text fontWeight={700} fontSize={"1.5rem"} gap="0.313rem" color={color}>
                ${totalTVL}
              </Text>
        ...
        <VStack borderRadius={"1rem"} p="1rem" gap="1.875rem" bg={boxBg}>
          <HStack w="100%" gap="0.375rem">
            <Box w={"3px"} h="0.875rem" bgColor={"#AB69FF"} borderRadius={"3px"}></Box>
            <HStack gap="0.2rem">
              <Text fontSize={"0.875rem"} color={color}>
                Volume
              </Text>
        ...
        <VStack borderRadius={"1rem"} p="1rem" gap="1.875rem" bg={boxBg}>
          <HStack w="100%" gap="0.375rem">
            <Box w={"3px"} h="0.875rem" bgColor={"#BDFF00"} borderRadius={"3px"}></Box>
            <HStack gap="0.2rem">
              <Text fontSize={"0.875rem"} color={color}>
                My Deposits
              </Text>
        ...
        <VStack borderRadius={"1rem"} p="1rem" gap="1.875rem" bg={boxBg}>
          <HStack w="100%" gap="0.375rem">
            <Box w={"3px"} h="0.875rem" bgColor={"#FFFFFF"} borderRadius={"3px"}></Box>
            <HStack gap="0.2rem">
              <Text fontSize={"0.875rem"} color={color}>
                My Rewards
              </Text>
          ...
          <VStack gap="1rem">
            <HStack w="100%" alignItems="baseline">
              <Text fontWeight={700} fontSize={"1.5rem"} gap="0.313rem" color={lightColor}>
                Coming soon
              </Text>
              {/* <Text fontWeight={400} fontSize={"0.875rem"} color={lightColor}>
                ETH
      ...
      <Box>
        <TableContainer borderLeftRadius={"1.25rem"} borderRightRadius={"1.25rem"}>
          <Table>
            <Thead bg={tableBg} h="3.125rem">
              <Tr>
                <Th fontSize={"0.75rem"} color={color} style={{ textTransform: "capitalize" }} fontWeight={500}>
                  Token
                </Th>
                <Th fontSize={"0.75rem"} color={color} style={{ textTransform: "capitalize" }} fontWeight={500}>
                  TVL
                ...
                return (
                  <Tr h="4.375rem" key={token?.id}>
                    <Td fontSize={"0.75rem"} color={color}>
                      <HStack gap="0.625rem">
                        <Image boxSize={"2.375rem"} src={token.img} alt="token" />
                        <VStack gap="0.25rem" alignItems={"flex-start"}>
                          <Text fontSize={"0.875rem"}>{token.name?.toUpperCase()}</Text>
                          <Text
                            fontSize={"0.5rem"}
                            fontWeight={400}
                            h={"0.75rem"}
